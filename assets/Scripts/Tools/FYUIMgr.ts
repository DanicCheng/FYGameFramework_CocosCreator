...
export default class FYUIMgr {
    ...
    /**
     * 预加载UI预制 路径是resources/Prefab/UI
     */
    public preLoad() {
        let self = this;
        cc.loader.loadResDir("Prefab/UI", cc.Prefab, function (completedCount, totalCount, item) {
    ...
    public show(uiName, parent = FYGlobalVarMgr.Instance.NodeTop, ...param) {
        ...
        if (this._dictUIPanel[uiName]) {
            ...
            this._dictUIPanel[uiName] = FYUtility.addChild(parent, this._dictUIPrefab[uiName]).getComponent(FYBaseUI);
        }
